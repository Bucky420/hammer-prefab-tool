...
export function moveBrushes(brushes, selected, delta, grid, snapResult = true){ brushes.forEach(b=>{if(!selected.has(b.id))return;b.vertices.forEach(v=>{v.x=snapResult ? snap(v.x+delta.x,grid) : v.x+delta.x;v.y=snapResult ? snap(v.y+delta.y,grid) : v.y+delta.y;v.z=snapResult ? snap(v.z+delta.z,grid) : v.z+delta.z})}); }
