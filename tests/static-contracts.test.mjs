  ...
  for (const [fileName, file] of files) {
    ...
    for (const statement of file.statements) {
      ...
      const optional = new Set(
        tags
          .filter((tag) => tag?.isBracketed)
          .map((tag) => tag.name.getText(file)),
      );
    ...
    for (const statement of file.statements) {
      ...
      for (const element of statement.importClause.namedBindings.elements) {
        ...
        if (required !== undefined)
          imported.set(element.name.text, { exportedName, required });
      }
    }
    const visit = (node) => {
  ...
  for (const entry of fs.readdirSync(directory, { withFileTypes: true })) {
    ...
    if (entry.isDirectory()) {
      ...
      files.set(
        ...
        sourceFile(fullPath, fs.readFileSync(fullPath, "utf8")),
      );
    }
  }
  return files;
...
const exportStart = appSource.indexOf("function savedVMFText");
const exportEnd = appSource.indexOf("const escapeHtml", exportStart);
assert.ok(
  appSource.slice(exportStart, exportEnd).includes("currentVMFDocument()"),
  "VMF save consumes the committed document only",
);
...
assert.ok(
  releaseStart >= 0 && releaseEnd > releaseStart,
  "extrusion release exists",
);
